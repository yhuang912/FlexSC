...
public class Paillier {
	...
	public void KeyGeneration(int bitLengthVal, int certainty) {
		...
		p = new BigInteger(bitLength / 2, certainty, CompEnv.rnd);
		q = new BigInteger(bitLength / 2, certainty, CompEnv.rnd);
	...
	public BigInteger Encryption(BigInteger m) {
		BigInteger r = new BigInteger(bitLength, CompEnv.rnd);
		return g.modPow(m, nsquare).multiply(r.modPow(n, nsquare)).mod(nsquare);
	...
	public static void main1(String[] str) {
		...
		for (int i = 0; i < iter_num; i++) {
			...
			BigInteger m1 = new BigInteger(paillier.bitLength, prng).mod(paillier.n);
			BigInteger m2 = new BigInteger(paillier.bitLength, prng).mod(paillier.n);
			/* encryption*/
		...
		System.out.println("Time per addition: " + total / (double)iter_num + " ms.");
	...
	public static void main(String[] str) {
		...
		/* instantiating an object of Paillier cryptosystem*/
		Paillier paillier = new Paillier();
		int fake_users_num = 10;
		int users_num = 5000000;

		/* instantiating two plaintext msgs*/
		BigInteger[] m = new BigInteger[fake_users_num];
		BigInteger[] em = new BigInteger[fake_users_num];
		for (int i = 0; i < fake_users_num; i++) {
			m[i] = new BigInteger(paillier.bitLength, prng).mod(paillier.n);
			em[i] = paillier.Encryption(m[i]);
		}
		BigInteger eres = em[0];
		BigInteger res = m[0];
		...
		for (int i = 0; i < users_num; i++) {
			...
			eres = eres.multiply(em[i % fake_users_num]).mod(paillier.nsquare);
			//	    res = res.add(m[i % fake_users_num]).mod(paillier.n);
		}
		System.out.println(users_num + "," + (System.currentTimeMillis() - startTime));
		startTime = System.currentTimeMillis();
		for (int i = 0; i < users_num; i++) {
			res = res.add(m[i % fake_users_num]).mod(paillier.n);
		}
		System.out.println((System.currentTimeMillis() - startTime) + " ms");
		eres = paillier.Decryption(eres);
		if (eres.equals(res)) {
