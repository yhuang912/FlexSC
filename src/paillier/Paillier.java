...
public class Paillier {
	...
	public static BigInteger encrypt(BigInteger plaintext, PublicKey pk) {
		...
		return tmp1.multiply(tmp2).mod(pk.modulous);
	}

	// Compute plaintext = L(c^(lambda) mod n^2) * mu mod n	
	public static BigInteger decrypt(BigInteger ciphertext, PrivateKey sk) {
		return  L(ciphertext.modPow(sk.lambda, sk.modulous), sk.n).multiply(sk.mu).mod(sk.n);
	}
	...
	public static BigInteger add(BigInteger ciphertext1, BigInteger ciphertext2, PublicKey pk){
		return ciphertext1.multiply(ciphertext2).mod(pk.modulous);		
	}

//	 On input two encrypted values, returns an encryption of the diff of the values
	    public static BigInteger sub(BigInteger ciphertext1, BigInteger ciphertext2, PublicKey pk){
	    	return add(ciphertext1, multiply(ciphertext2, -1, pk), pk);
	    }
	...
	public static BigInteger multiply(BigInteger ciphertext1, BigInteger scalar, PublicKey pk){
		return ciphertext1.modPow(scalar, pk.modulous);
	}
