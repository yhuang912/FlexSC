...
public class TestOram {
	...
	class GenRunnable extends network.Server  implements Runnable{
		...
		public void run() {
			try {
				listen(54321);
				RecursiveCircuitOram<GCSignal> client = new RecursiveCircuitOram<GCSignal>(is, os, N, dataSize, Mode.REAL, 80, Party.Alice);
	...
	class EvaRunnable extends network.Client implements Runnable{
		...
		public void run() {
			try {
				...
				RecursiveCircuitOram<GCSignal> server = new RecursiveCircuitOram<GCSignal>(is, os, N, dataSize, Mode.REAL, 80, Party.Bob);
				for(int i = 0; i < N; ++i) {
					int element = i%N;
