...
public class TestSparseMatrix<T> extends TestHarness<T> {
	...
	public abstract class Helper {
		...
		public abstract MatrixNode<T>[] secureCompute(MatrixNode<T>[] a, MatrixNode<T>[] b,
				SparseMatrixLib<T> lib) throws Exception;
	...
	class GenRunnable extends network.Server implements Runnable {
		...
		public void run() {
			try {
				...
				if (testFixedPoint)
					slib = new SparseMatrixLib<T>(new FixedPointLib<T>(gen, len, offset));
				else
					slib = new SparseMatrixLib<T>(new FloatLib<T>(gen, VLength, PLength));
...
//				PrintMatrix(h.b);
				...
				for (int i = 0; i < h.va.length; ++i) {
					if(i < h.acnt)
						fgc1[cnt1++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], false);
					else fgc1[cnt1++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], true);
				}

				for (int i = 0; i < h.vb.length; ++i)
					if(i < h.bcnt)
						fgc2[cnt2++] = slib.inputOfAlice(h.xb[i], h.yb[i], h.vb[i], false);
					else 
						fgc2[cnt2++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], true);



				//				MatrixNode<T>[] re = h.secureCompute(Arrays.copyOf(fgc1, h.acnt), Arrays.copyOf(fgc2, h.bcnt), slib);
	...
	class EvaRunnable extends network.Client implements Runnable {
		...
		public void run() {
			try {
				...
				SparseMatrixLib<T> slib;
				if (testFixedPoint)
					slib = new SparseMatrixLib<T>(new FixedPointLib<T>(env, len, offset));
				else
					slib = new SparseMatrixLib<T>(new FloatLib<T>(env, VLength, PLength));
				...
				for (int i = 0; i < h.va.length; ++i) {
					if(i < h.acnt)
						fgc1[cnt1++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], true);
					else fgc1[cnt1++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], false);
				}

				for (int i = 0; i < h.vb.length; ++i)
					if(i < h.bcnt)
						fgc2[cnt2++] = slib.inputOfAlice(h.xb[i], h.yb[i], h.vb[i], true);
					else 
						fgc2[cnt2++] = slib.inputOfAlice(h.xa[i], h.ya[i], h.va[i], false);
		...
		if (m == Mode.COUNT) {
			...
			for (int i = 0; i < result.length; ++i)
				for (int j = 0; j < result[0].length; ++j) {
					double error = 0;
					if (gen.z[i][j] != 0)
						error = Math.abs((result[i][j] - gen.z[i][j]) / gen.z[i][j]);
					else
