...
public class BF<t__T> implements IWritable<BF<t__T>, t__T> {

	public CompEnv<t__T> env;
	public IntegerLib<t__T> intLib;
	public FloatLib<t__T> floatLib;

	public BF(CompEnv<t__T> env) throws Exception {
		this.env = env;
	...
	public BF<t__T> newObj(t__T[] data) throws Exception {
		if(data == null) {
			data = env.newTArray(this.numBits());
			for(int i=0; i<this.numBits(); ++i) { data[i] = intLib.SIGNAL_ZERO; }
		}
		if(data.length != this.numBits()) return null;
		BF<t__T> ret = new BF<t__T>(env);
		t__T[] tmp;
		int now = 0;
		return ret;
}
