...
KeyValue@m<T> PriorityQueue@m<T>.pqueue_op(ints_16 key, T operand, ints_1 op) {
   ...
   if(op == POP){
      ints_1 insert_dummy_bit = false;
      ints_1 pop_dummy_bit = true;
   }
   else {
...
KeyValue@m<T> PriorityQueue@m<T>.pop(ints_1 dummy_bit) {
   KeyValue@m<T> ret = this.extractMax(dummy_bit);
   return ret;
}
...
ints_1 PriorityQueue@m<T>.should_go_left(ints_@m id, intp_@m level) {
   ints_1 ret;
   if(level >= m - 1)
      ret = true;
   else {
...
KeyValue@m<T> PriorityQueue@m<T>.extractMax(ints_1 dummy_bit) {
   KeyValue@m<T> ret;
   PriorityQueueNode@m<T> last_node = this.get_last(this.root, 1, dummy_bit);
   ints_1 newdummy_bit = false;
   if(dummy_bit == true) {
...
intr_@m PriorityQueue@m<T>.heapify(ints_@m top_id, PriorityQueueNode@m<T> top_node, intp_@m level, ints_1 dummy_bit) {
   intr_@m ret;
   ints_1 newdummy_bit = false;
   ints_1 go_left = false;
   intr_@m child_iter;
   ints_@m id_to_use;
   intr_@m pos_to_use;
...
PriorityQueueNode@m<T> PriorityQueue@m<T>.get_last(NodeId@m top, intp_@m level, ints_1 dummy_bit) {
   ...
   if(level < m) {
      if(dummy_bit == true && top.id <= this.size) {
         ...
         if(top.id == this.size) {
            ...
            ints_1 go_left = this.should_go_left(this.size, level);
...
void PriorityQueue@m<T>.insert(KeyValue@m<T> kv, ints_1 dummy_bit) {
   KeyValue@m<T> parent_kv;
   parent_kv.key = 1<<15-1;
   if(dummy_bit == true) {
      this.size = this.size + 1;
...
intr_@m PriorityQueue@m<T>.insert_internal(KeyValue@m<T> kv, KeyValue@m<T> parent_kv,
      ints_@m iter_id, intr_@m iter_pos, intp_@m level, ints_1 dummy_bit) {
   intr_@m ret;
   ints_1 newdummy_bit = false;
   if(level < m) {
