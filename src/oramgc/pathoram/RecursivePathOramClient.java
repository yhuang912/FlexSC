...
public class RecursivePathOramClient {
	TrivialOramClient baseOram;
	ArrayList<PathOramClient> clients = new ArrayList<>();
	...
	public RecursivePathOramClient(InputStream is, OutputStream os, int N, int dataSize, int cutoff, int recurFactor, int capacity) throws Exception {
		this.is = is;
		this.os = os;
		this.cutoff = cutoff;
		this.recurFactor = recurFactor;
		this.capacity = capacity;
		PathOramClient  oram = new PathOramClient(is, os, N, dataSize, Party.CLIENT);
		clients.add(oram);
		...
		while(newN > cutoff) {
			...
			oram = new PathOramClient(is, os, newN, newDataSize, Party.CLIENT);
			clients.add(oram);
		}
		PathOramClient last = clients.get(clients.size()-1);
	...
	public boolean[] read(int iden) throws Exception {
		...
		PathOramClient currentOram = clients.get(0);
	...
	public boolean[][] travelToDeep(int iden, int level) throws Exception {
		if(level == clients.size()) {
			...
			PathOramClient currentOram = clients.get(level);
