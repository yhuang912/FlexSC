...
public class CLPOramRecClient<T> {
	TrivialOramClient<T> baseOram;
	public ArrayList<CLPOramBasicClient<T>> clients = new ArrayList<>();
	...
	public boolean[] read(int iden) throws Exception {
		...
		CLPOramBasicClient<T> currentOram = clients.get(0);
	...
	public boolean[][] travelToDeep(int iden, int level) throws Exception {
		if(level == clients.size()) {
			...
			CLPOramBasicClient<T> currentOram = clients.get(level);
