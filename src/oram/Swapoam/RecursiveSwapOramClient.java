...
public class RecursiveSwapOramClient<T> {
	TrivialOramClient<T> baseOram;
	public ArrayList<SwapOramClient<T>> clients = new ArrayList<>();
	...
	public RecursiveSwapOramClient(InputStream is, OutputStream os, int N, int dataSize, int cutoff, int recurFactor, 
		...
		SwapOramClient<T>  oram = new SwapOramClient<T>(is, os, N, dataSize, Party.Alice, capacity, m, sp);
		clients.add(oram);
		...
		while(newN > cutoff) {
			...
			oram = new SwapOramClient<T>(is, os, newN, newDataSize, Party.Alice, capacity, m, sp);
			clients.add(oram);
		}
		SwapOramClient<T> last = clients.get(clients.size()-1);
	...
	public boolean[] read(int iden) throws Exception {
		...
		SwapOramClient<T> currentOram = clients.get(0);
	...
	public boolean[][] travelToDeep(int iden, int level) throws Exception {
		if(level == clients.size()) {
			...
			SwapOramClient<T> currentOram = clients.get(level);
