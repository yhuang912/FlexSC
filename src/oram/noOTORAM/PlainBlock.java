...
public class PlainBlock {
	...
	public PlainBlock(boolean[] iden, boolean[] pos, boolean[] data,
			boolean isDummy) {
		this.iden = iden;
		this.pos = pos;
		this.data = data;
	...
	public PlainBlock(boolean[] d, int lengthOfIden, int lengthOfPos,
			int lengthOfData) {
		this.iden = Arrays.copyOfRange(d, 0, lengthOfIden);
		this.pos = Arrays.copyOfRange(d, lengthOfIden, lengthOfIden
				+ lengthOfPos);
		this.data = Arrays.copyOfRange(d, lengthOfIden + lengthOfPos,
				lengthOfIden + lengthOfPos + lengthOfData);
		this.isDummy = d[d.length - 1];
	}

	public boolean[] toBooleanArray() {
		boolean[] result = new boolean[iden.length + pos.length + data.length
				+ 1];
