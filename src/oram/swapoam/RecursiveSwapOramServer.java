...
public class RecursiveSwapOramServer<T> {
	public TrivialOramServer<T> baseOram;
	public ArrayList<SwapOramServer<T>> servers = new ArrayList<>();
	int recurFactor;
	int cutoff;
	int capacity;
	...
	public RecursiveSwapOramServer(InputStream is, OutputStream os, int N, int dataSize, int cutoff, int recurFactor, 
		...
		SwapOramServer<T>  oram = new SwapOramServer<T>(is, os, N, dataSize, Party.Bob, capacity,  m, sp);
		servers.add(oram);
		...
		while(newN > cutoff) {
			...
			oram = new SwapOramServer<T>(is, os, newN, newDataSize, Party.Bob, capacity,  m, sp);
			servers.add(oram);
		}
		SwapOramServer<T> last = servers.get(servers.size()-1);
		baseOram = new TrivialOramServer<T>(is, os, (1<<last.lengthOfIden), last.lengthOfPos, m);
	}
	
	public void access() throws Exception {
		travelToDeep(1);
		SwapOramServer<T> currentOram = servers.get(0);
	...
	public void travelToDeep(int level) throws Exception {
		if(level == servers.size()) {
			...
			SwapOramServer<T> currentOram = servers.get(level);
			boolean[] pos = getBooleans();
