...
public class RecursiveCSCOramServer<T> {
	public TrivialOramServer<T> baseOram;
	public ArrayList<CSCOramServer<T>> servers = new ArrayList<>();
	int recurFactor;
	int cutoff;
	int capacity;
	...
	public RecursiveCSCOramServer(InputStream is, OutputStream os, int N, int dataSize, int cutoff, int recurFactor, 
...
//		System.out.print(capacity);
		...
		while(newN > cutoff) {
			...
			oram = new CSCOramServer<T>(is, os, newN, newDataSize, Party.Bob, capacity,  m, sp);
			servers.add(oram);
		}
		CSCOramServer<T> last = servers.get(servers.size()-1);
		baseOram = new TrivialOramServer<T>(is, os, (1<<last.lengthOfIden), last.lengthOfPos, m);
	}
	
	public void access() throws Exception {
		travelToDeep(1);
		CSCOramServer<T> currentOram = servers.get(0);
	...
	public void travelToDeep(int level) throws Exception {
		if(level == servers.size()) {
			...
			CSCOramServer<T> currentOram = servers.get(level);
			boolean[] pos = getBooleans();
