...
public class RecursiveCSCOramClient<T> {
	...
	public ArrayList<CSCOramClient<T>> clients = new ArrayList<>();
	...
	public RecursiveCSCOramClient(InputStream is, OutputStream os, int N, int dataSize, int cutoff, int recurFactor, 
		...
		CSCOramClient<T>  oram = new CSCOramClient<T>(is, os, N, dataSize, Party.Alice, capacity, m, sp);
		...
		while(newN > cutoff) {
			...
			oram = new CSCOramClient<T>(is, os, newN, newDataSize, Party.Alice, capacity, m, sp);
			clients.add(oram);
		}
		CSCOramClient<T> last = clients.get(clients.size()-1);
	...
	public boolean[][] travelToDeep(int iden, int level) throws Exception {
		if(level == clients.size()) {
			...
			CSCOramClient<T> currentOram = clients.get(level);
