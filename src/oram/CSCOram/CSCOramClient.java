...
public class CSCOramClient<T> extends CSCOramParty<T> {
	CSCOramLib<T> lib;
	public CSCOramClient(InputStream is, OutputStream os, int N, int dataSize,
			Party p, int cap, Mode m, int sp) throws Exception {
		super(is, os, N, dataSize, p, cap, m, sp);
		lib = new CSCOramLib<T>(lengthOfIden, lengthOfPos, lengthOfData, logN, capacity, gen);
	...
	public void flushOneTime(boolean[] pos) throws Exception {
		...
		PlainBlock[][] blocks = preparePlainPath(globalSCPath[0], globalSCPath[1]);
		putPath(blocks, pos);
	}
