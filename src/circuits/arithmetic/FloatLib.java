...
public class FloatLib<T> implements ArithmeticLib<T>{
	...
	public T[] multiply(T[] fa, T[] fb) throws Exception {
		Representation<T> a = unpack(fa);
		Representation<T> b = unpack(fb);

		T new_s = lib.xor(a.s, b.s);
		T[] a_multi_b = lib.unSignedMultiply(a.v, b.v);//length 2*v.length
	...
	public T[] div(T[] fa, T[] fb) throws Exception {
		Representation<T> a = unpack(fa);
		Representation<T> b = unpack(fb);
	...
	public T[]addInternal(T sa, T sb, T[] va, T[] vb, T[] p, T[] pDiff) throws Exception {
	...
	public T[] add(T[] fa, T[] fb) throws Exception {
		...
		T[] normalCase = addInternal(lib.mux(fa[0], fb[0],paGreater),lib.mux(fb[0], fa[0],paGreater),  lib.mux(va, vb, paGreater), lib.mux(vb, va, paGreater), pToUse, pDiffAbs);
	...
	// (v*s^p)^(1/2) = 
	public T[] sqrt(T[] fa) throws Exception {
...
//		pa = lib.conditionalDecrement(pa, pa[0]);
		...
		va = lib.sqrt(va);
		pa = lib.rightPublicShift(pa, 1);pa[pa.length-1] = pa[pa.length-2];
	...
	public T[] sub(T[] a, T[] b) throws Exception {
