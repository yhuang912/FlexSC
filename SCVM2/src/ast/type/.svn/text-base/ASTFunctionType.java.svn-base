...
public class ASTFunctionType extends ASTType {
	...
	public ASTFunctionType(ASTType ret, String name, List<String> bitParameter, List<ASTType> inputs, boolean global) {
	...
	public boolean match(ASTType type) {
		...
		if(!returnType.match(ftype.returnType) || this.inputTypes.size() != ftype.inputTypes.size())
			return false;
		for(int i=0; i<inputTypes.size(); ++i)
			if(!this.inputTypes.get(i).match(ftype.inputTypes.get(i)))
				return false;
		return true;
	}
	@Override
